...
//go:generate mockgen -destination=../../mocks/mock_uow.go -package=mocks github.com/surgged/agni/service/uow UnitOfWork
//go:generate mockgen -destination=../../mocks/mock_tx_repositories.go -package=mocks github.com/surgged/agni/service/uow TxRepositories

import (
	"context"

	"github.com/surgged/agni/domain/shared"
	"github.com/surgged/agni/domain/user"

	// crank:tx-repo-imports (do not remove — `crank make scaffold` splices new domain imports here)
	domainapp "github.com/surgged/agni/domain/app"
	domainsharelink "github.com/surgged/agni/domain/sharelink"
)
