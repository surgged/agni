...
import (
	...
	appapp "github.com/surgged/agni/service/app"
	"github.com/surgged/agni/service/auth/agentapikey"
	"github.com/surgged/agni/service/deploy"
)
...
func NewAppHandler(cmd *appapp.CommandHandler, qry *appapp.QueryHandler, deploySvc *deploy.Service, userTokens ports.TokenService, agentTokens *agentapikey.AgentTokenService) *AppHandler {
	return &AppHandler{
		cmd:         cmd,
		qry:         qry,
		deploySvc:   deploySvc,
...
//	@Failure      429   {object}  rest.Error
//	@Router       /api/v1/apps [post]
func (h *AppHandler) Create(c *echo.Context) error {
	email := h.extractEmail(c)
	if email == "" {
		return c.JSON(http.StatusUnauthorized, rest.Error{Error: "authentication required"})
	}
	...
	if dto.Name == "" {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "name is required"})
	}
...
func (h *AppHandler) Get(c *echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusNotFound, rest.Error{Error: "app not found"})
	}
...
func (h *AppHandler) Logs(c *echo.Context) error {
	...
	if _, err := uuid.Parse(appID); err != nil {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "invalid app id"})
	}
...
func (h *AppHandler) IssueAgentToken(c *echo.Context) error {
	email := h.extractEmail(c)
	if email == "" {
		return c.JSON(http.StatusUnauthorized, rest.Error{Error: "authenticated session required"})
	}
	...
	if err != nil {
		...
		return c.JSON(http.StatusInternalServerError, rest.Error{Error: "failed to issue agent token"})
	}
...
func (h *AppHandler) Preview(c *echo.Context) error {
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		return c.JSON(http.StatusNotFound, rest.Error{Error: "app preview data not found"})
	}
...
func (h *AppHandler) MultipartInit(c *echo.Context) error {
	...
	if err := c.Bind(&req); err != nil {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "invalid request: " + err.Error()})
	}
	if req.TotalSize <= 0 {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "total_size must be positive"})
	}
...
func (h *AppHandler) MultipartComplete(c *echo.Context) error {
	...
	if req.UploadID == "" {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "upload_id is required"})
	}
	if len(req.Parts) == 0 {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "parts list cannot be empty"})
	}
