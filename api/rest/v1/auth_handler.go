...
import (
	...
	"github.com/surgged/agni/api/rest/middleware"
	domainUser "github.com/surgged/agni/domain/user"
	"github.com/surgged/agni/ports"
	"github.com/surgged/agni/service/user"
)
...
func (h *AuthHandler) RegisterUser(c *echo.Context) error {
	...
	if in.Password != in.ConfirmPassword {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "passwords do not match"})
	}
...
func (h *AuthHandler) Login(c *echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusUnauthorized, rest.Error{Error: "invalid credentials"})
	}
...
func (h *AuthHandler) VerifyEmail(c *echo.Context) error {
	...
	if token == "" {
		return c.JSON(http.StatusBadRequest, rest.Error{Error: "verification token is required"})
	}
	ctx := c.Request().Context()
...
func (h *AuthHandler) Refresh(c *echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusUnauthorized, rest.Error{Error: err.Error()})
	}
	return c.JSON(http.StatusOK, tokenResponse{
