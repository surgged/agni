...
import (
	...
	"github.com/surgged/agni/api/validator"
)
...
func NewServer(logger *slog.Logger) *echo.Echo {
	...
	e.HTTPErrorHandler = func(c *echo.Context, err error) {
		...
		if ve, ok := err.(*validator.ValidationError); ok {
			...
			if jsonErr := c.JSON(ve.HTTPStatus, Error{Error: ve.Message, Details: ve.Errors}); jsonErr != nil {
		...
		if code := echo.StatusCode(err); code != 0 {
			...
			if jsonErr := c.JSON(code, Error{Error: msg}); jsonErr != nil {
		...
		if jsonErr := c.JSON(http.StatusInternalServerError, Error{Error: "internal server error"}); jsonErr != nil {
